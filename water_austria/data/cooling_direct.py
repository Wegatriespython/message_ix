...
def add_cooling_direct(scenario: Scenario, **options) -> None:
    """
...
def _get_parent_technologies(scenario: Scenario) -> List[str]:
    ...
    output_df = scenario.par("output", filters={
        "commodity": "electricity",
        "level": ["secondary", "final"]
    })
...
def _add_cooling_technology_variants(scenario: Scenario, parent_techs: List[str]) -> List[str]:
...
def _add_cooling_data(scenario: Scenario, parent_techs: List[str], 
                      cooling_techs: List[str], info: ScenarioInfo) -> None:
    ...
    for cooling_tech in cooling_techs:
        ...
        lifetime_df = make_df("technical_lifetime",
            node_loc=node,
    ...
    if "extract_freshwater" not in scenario.set("technology"):
        ...
        # Technical lifetime
        water_lifetime = make_df("technical_lifetime",
            node_loc=node,
            technology="extract_freshwater",
            year_vtg=info.Y,
            value=60,
        ...
        # Capacity factor
        water_cf = make_df("capacity_factor",
            node_loc=node,
            technology="extract_freshwater",
            year_vtg=info.Y,
            year_act=info.Y,
